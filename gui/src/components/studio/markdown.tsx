...
function renderProse(block: string, nodes: ReactNode[], ctx: MdCtx) {
      ...
      <ul key={`k${nodes.length}`} className="my-2 space-y-1 pl-0">
        {tasks.map((item, idx) => (
          <li key={idx} className="flex items-start gap-2">
            <input type="checkbox" checked={item.checked} readOnly disabled className="mt-1 size-3.5 shrink-0" />
            <span className={item.checked ? "text-muted-foreground line-through" : ""}>{inline(item.text, ctx)}</span>
          </li>
        ))}
      </ul>,
    );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (heading) {
      ...
      nodes.push(
        <Tag key={`h${nodes.length}`} id={ctx.slug(raw)} className={HEADING_CLASS[level - 1]}>
          {inline(raw, ctx)}
        </Tag>,
      );
      continue;
...
export function Markdown({ text, className }: { text: string; className?: string }) {
  ...
  if (notes.size) {
    ...
    nodes.push(
      <ol key="fn-list" className="my-2 list-decimal space-y-1 pl-5 text-xs text-muted-foreground">
  ...
  return (
    <div ref={rootRef} className={cn("text-pretty text-sm text-foreground/90", className)}>
