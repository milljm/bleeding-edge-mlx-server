...
- `GET /v1/hub` — `{token, help}` whether `HF_TOKEN` / `HUGGING_FACE_HUB_TOKEN`
  is set (the value is never returned). Search/download require a token.
  `POST /v1/hub/search` `{query}` lists MLX quants. `POST /v1/hub/download`
  `{repo}` starts `snapshot_download` in a child process. Several repos can
  run at once (same repo is a no-op). Poll `GET /v1/hub/progress` for `{jobs}`
  (byte-level ratio + `pct` + `detail`). Pause is SIGSTOP. Resume is SIGCONT.
...
Some checkpoints stream every token on `reasoning` / `reasoning_content` and,
with no think tags, copy the same text into `content` at the end — clients
that only read `delta.content` see a dump. Settings → Thinking → **Reason as
response** folds the reasoning channel into `content` as tokens arrive and
drops the trailing replay (including mlx-lm's `delta.reasoning` field). It is
an Edge pref (not an mlx-lm flag); it takes effect on the next request without
Reload.
